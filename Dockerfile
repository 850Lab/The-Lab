...
# Same-origin API+SPA (Railway single service): multipart upload hits this server — avoids browser→S3 presigned PUT (CORS/storage gaps). Split hosting with S3: pass --build-arg VITE_USE_DIRECT_REPORT_UPLOAD=1.
ARG VITE_USE_DIRECT_REPORT_UPLOAD=0
...
ENV VITE_USE_DIRECT_REPORT_UPLOAD=$VITE_USE_DIRECT_REPORT_UPLOAD
