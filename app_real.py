...
from services.dedupe_helpers import compute_deduped_counts
from services.review_helpers import review_claim_to_canonical
from services.ux_metrics import compute_uxfs_report
...
if 'reports_processed' not in st.session_state:
    st.session_state.reports_processed = 0
if 'upload_file_skips' not in st.session_state:
    st.session_state.upload_file_skips = []
            ...
            if valid_files and st.button("Extract & Analyze All", type="primary", use_container_width=True):
              try:
                ...
                upload_options = {
                    ...
                    "dev_mode": DEV_MODE,
                    "on_report_saved": lambda bureau, _fn: track_analytics(
                        "upload", "UPLOAD", f"bureau:{bureau}"
                    ),
                }
                ...
                with st.status("Extracting and analyzing your credit report(s)...", expanded=True) as _upload_status:
                    _upload_status.update(label="Processing uploaded PDFs...", state="running")
                    st.write("Reading files, detecting bureau, parsing accounts and building review claims...")
                    result = process_uploaded_reports(pdf_files, upload_options)
                    _upload_status.update(label="Analysis complete", state="complete")

                for skip_entry in result.get("file_skips", []):
                    fn = skip_entry.get("filename", "")
                    reason = skip_entry.get("reason", "")
                    if reason == "3bureau":
                        st.warning(
                            f"⚠️ **{fn}** is a combined 3-bureau report (e.g., SmartCredit). "
                            "Please upload individual single-bureau reports from TransUnion, Experian, and Equifax for accurate parsing."
                        )
                    elif reason == "unknown":
                        st.warning(
                            f"⚠️ **{fn}** — Could not identify the credit bureau. "
                            "Please verify this is a TransUnion, Experian, or Equifax credit report."
                        )

                st.session_state.uploaded_reports = result["uploaded_reports"]
                st.session_state.extracted_claims = result["extracted_claims"]
                st.session_state.review_claims = result["review_claims"]
                st.session_state.claim_responses = {}
                st.session_state.review_claim_responses = {}
                st.session_state.identity_confirmed = {}
                st.session_state.generated_letters = {}
                ...
                st.session_state.capacity_selection = {}
                st.session_state.system_error_message = None
                st.session_state.upload_diagnostics = result["diagnostics"]
                st.session_state.review_incomplete_ack = False
                st.session_state.reports_processed = result["reports_processed"]
                st.session_state.upload_file_skips = result.get("file_skips", [])

                if result["reports_processed"] > 0:
                    st.session_state.current_report = list(st.session_state.uploaded_reports.values())[0]
                    st.session_state.completeness_report = result["completeness_report"]
                    st.session_state.totals_confidence = result["totals_confidence"]
                    st.session_state.exactness = result["exactness"]
                    st.session_state.report_totals = result["report_totals"]
                    st.session_state.parsed_totals = result["parsed_totals"]
                    st.session_state.review_exactness_state = result["review_exactness_state"]
                    st.session_state.unified_summary = result["unified_summary"]
                    if result["plain_text"] is not None:
                        st.session_state["plain_text"] = result["plain_text"]
                    if (DEV_MODE or DEBUG_MODE) and result["layout"] is not None:
